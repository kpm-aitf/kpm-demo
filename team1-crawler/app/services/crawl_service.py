...
def make_batch_id() -> str:
    """Generate a batch_id in the format: batch-YYYYMMDD-HHhMMss (UTC+7)."""
    import pytz
    tz = pytz.timezone("Asia/Jakarta")
    now = datetime.now(tz=tz)
    return now.strftime("batch-%Y%m%d-%Hh%M%S")
...
async def run_batch(
    ...
    try:
        ...
        log.error("batch_fatal_error", error=str(exc))
        batch_status = "FAILED"
        total_errors += 1

    finally:
        # ── Complete batch manifest — always runs ──
        if not dry_run:
            success_rate = (
